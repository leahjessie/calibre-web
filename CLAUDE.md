...
`run/stable` and `run/canary` are build artifacts. Make changes in `dev/` on the appropriate `bug/` or `feat/` branch, then rebuild via `build.sh`.
...
When a `pr/xxx` is accepted upstream: rebase `base` onto `master`, move the test file from `bug/xxx` into `base`, remove the branch from the profile conf files in `meta/profiles/`, rebuild. Full steps in WORKFLOW.md.
