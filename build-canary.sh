#!/usr/bin/env bash
set -euo pipefail

DEV_DIR=~/Developer/calibre-web/dev
LAB_DIR=~/Developer/calibre-web/lab

merge_branch() {
    local branch="$1"
    git -C "$DEV_DIR" merge --no-ff "$branch" || {
        if ! git -C "$DEV_DIR" rev-parse MERGE_HEAD &>/dev/null; then
            echo "Failed to merge $branch (branch missing or other error)" >&2; exit 1
        fi
        if [[ -n $(git -C "$DEV_DIR" ls-files -u) ]]; then
            echo "Unresolved conflicts merging $branch — fix manually then re-run" >&2; exit 1
        fi
        echo "rerere auto-resolved conflicts in $branch — committing"
        git -C "$DEV_DIR" commit --no-edit
    }
}

# Tag the current run/canary before overwriting it (idempotent — skip if tag exists)
git -C "$DEV_DIR" tag "run/canary-$(date +%Y%m%d)" run/canary 2>/dev/null || true

# Remember dev/'s current branch so we can restore it after building
DEV_BRANCH=$(git -C "$DEV_DIR" branch --show-current)

# Detach HEAD in lab/ and dev/ to free up run/canary for rebuilding
git -C "$LAB_DIR" checkout --detach HEAD
git -C "$DEV_DIR" checkout --detach HEAD

# Build run/canary: start from base, merge all active branches
git -C "$DEV_DIR" checkout -B run/canary base
merge_branch bug/kobo-sync

# Add additional bug/ and feat/ branches here as they are created:
# merge_branch bug/kobo-mode-switch
# merge_branch feat/series-author-add-shelf

# Restore both worktrees: dev/ back to its original branch, lab/ to the new build
git -C "$DEV_DIR" checkout "$DEV_BRANCH"
git -C "$LAB_DIR" checkout run/canary

echo "run/canary built successfully"
